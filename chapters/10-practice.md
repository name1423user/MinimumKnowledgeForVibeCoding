...
7. **壊し方をテスト化する** — 見つけたバグをテストコードとして残す（[5.2節](05-breaking-it.md#52-壊し方をテストコードに変換する)）
...
見つかったバグを直し、そのまま[5.2節](05-breaking-it.md#52-壊し方をテストコードに変換する)の要領でテストにする。
...
- **CI**（[第8章](08-testing-and-cicd.md)）: 上のテストをpushのたびに自動実行する設定を追加する（[8.2節のGitHub Actions例](08-testing-and-cicd.md#82-ci継続的インテグレーションとは)がそのまま使える）
- **ログ**（[第6章](06-noticing.md)）: `validate_note`が`ValueError`を投げた時、どんな入力で失敗したかを記録する。この時WARNINGかERRORか、レベルの選び方（[6.1.1節](06-noticing.md#611-ログレベルの使い分け)）も意識する
- **セキュリティ**（[第7章](07-security.md)）: `escape()`でXSSを防いだ他に、メモの長さに上限（例: 1万文字）を設けて巨大データ攻撃も防ぐ。もし将来「メモに添付ファイルのパスを持たせる」機能を足すなら、[7.2.2節「パストラバーサル」](07-security.md#722-パストラバーサル)の検証も必要になる
- **設計**（[第3章](03-design.md)）: `validate_note` / `save_note` / 表示処理が分離済みなので、次に「メモを編集する」機能を足す時も影響範囲が予測しやすい

このエンドポイントを本番相当に育てるなら、あと2つ検討課題が残る。1つは`notes`がグローバルなPythonのリストのままだと、[5.1.1節「同時実行を実際に手元で再現する」](05-breaking-it.md#511-同時実行を実際に手元で再現する)で見た通り、複数リクエストが同時に来た時に書き込みが競合しうる点（本番ではDBのトランザクションに置き換えることになる）。もう1つは、CLI・エージェント型のツールでこの実装を任せている場合、外部から取得したテキスト（他サービスからインポートしたメモなど）を無警戒に処理させると[9.3.1節「プロンプトインジェクション」](09-choosing-ai-tools.md#931-エージェント型特有のリスク-プロンプトインジェクション)と同種の注意が必要になる点だ。小さなサンプルほど「ここまでで十分」と感じやすいが、本番運用を意識すると検討項目は際限なく増える。どこまでやるかは、そのアプリが晒すリスクの大きさで判断する。
