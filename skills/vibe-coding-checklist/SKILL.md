...
凡例: 各項目の判定は、実行したそれぞれのskillの凡例に従う（迷ったら該当skillのSKILL.mdを開く）。「済/未/差し戻し中」は、その項目が完了しているか、まだ対応が要るか、仕様確定待ちで止まっているかを表す。
...
## 次にすること
CRITICALがあれば最優先でそこだけ対応する。無ければ「総評」に書かれた順番で着手する。個々の項目の判定に納得できない場合は、その項目を出したskillのSKILL.mdと対応章に戻って確認する。
